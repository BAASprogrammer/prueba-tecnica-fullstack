import { ChevronLeft, ChevronRight, Pencil, XCircle, Trash2, Loader2 } from 'lucide-react';
import { requestStatus } from '../constants/requestStatus';
import type { PropsRequestsTable } from '../types/request';

export default function RequestsTable({
  requests, loading, total, page, totalPages,
  expanded, disabledEditIds,
  onPageChange, onToggleExpand, onCloseRequest, onDeleteRequest, onEditRequest,
}: PropsRequestsTable) {
  return (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 max-w-xs border-b border-gray-200">
                      <button
                        onClick={() => onToggleExpand(expanded === r.id ? null : r.id)}
                    ...
                    <td className="px-4 py-2.5 whitespace-nowrap border-b border-gray-200">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => onEditRequest(r.id)}
                          disabled={disabledEditIds.has(r.id)}
                          className="text-gray-400 hover:text-blue-600 transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
                          title={disabledEditIds.has(r.id) ? 'Siendo editada en otra pestaña' : 'Editar solicitud'}
                        ...
                        <button
                          ...
                          disabled={r.status === 'FINALIZADA'}
                          className="text-gray-400 hover:text-emerald-600 transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
                          title={r.status === 'FINALIZADA' ? 'Solicitud ya cerrada' : 'Cerrar solicitud'}
            ...
            <div className="flex items-center justify-center gap-1.5 px-4 py-2.5 border-t border-gray-200 bg-gray-50/50">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button
                  ...
                  className={`w-8 h-8 text-xs font-medium rounded-md transition-colors cursor-pointer ${p === page
