...
import type { RequestFormModalProps, FieldErrors } from '../types/request-form';
import { validateRequestForm } from '../hooks/useRequests';
import { useClientAutocomplete } from '../hooks/useClientAutocomplete';

// Modal unificado de creación/edición de solicitud
export default function RequestFormModal({ mode, initialData, onSave, onClose }: RequestFormModalProps) {
  ...
  // Autocompletado: al recibir datos desde el hook, solo llena si el campo está vacío
  const fillResult = (result: { name: string; email: string; phone: string }) => {
    setClientName((prev) => prev || result.name);
    setClientEmail((prev) => prev || result.email);
    setClientPhone((prev) => prev || result.phone);
  };

  // Hook de autocompletado por RUT o email (solo en creación)
  const { autoLoading, autoError } = useClientAutocomplete(rut, clientEmail, fillResult, !isEdit);

  // Guarda la solicitud con validación previa
  const handleSave = async () => {
    // Valida el formulario
    const validation = validateRequestForm({ clientName, clientEmail, clientPhone, date, type, description });
    setErrors(validation);
    // Si hay errores, no guarda
    if (Object.keys(validation).length > 0) return;
    setSaving(true);
    try {
      await onSave({ id: initialData?.id, date, type, description, status, clientName, clientEmail, clientPhone });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">
            {isEdit ? <>Editar solicitud <span className="text-indigo-600">{initialData!.number}</span></> : 'Nueva solicitud'}
          </h3>
        ...
        {autoError && (
          <div className="mb-4 text-sm text-red-600 bg-red-50 px-3 py-2 rounded-lg">{autoError}</div>
        )}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input value={clientName} onChange={(e) => { setClientName(e.target.value); clearError('clientName'); }} className={inputClass(errors.clientName)} />
            {errors.clientName && <p className="text-xs text-red-500 mt-1">{errors.clientName}</p>}
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Email</label>
            <input value={clientEmail} onChange={(e) => { setClientEmail(e.target.value); clearError('clientEmail'); }} className={inputClass(errors.clientEmail)} />
          ...
          <div>
            ...
            <input value={clientPhone} onChange={(e) => { setClientPhone(e.target.value); clearError('clientPhone'); }} placeholder="+56912345678" className={inputClass(errors.clientPhone)} />
          ...
          <div>
            ...
            <input type="date" value={date} min={!isEdit ? today : undefined} onChange={(e) => { setDate(e.target.value); clearError('date'); }} className={inputClass(errors.date)} />
          ...
          <div>
            ...
            <input value={type} onChange={(e) => { setType(e.target.value); clearError('type'); }} className={inputClass(errors.type)} />
            {errors.type && <p className="text-xs text-red-500 mt-1">{errors.type}</p>}
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Descripción</label>
            <textarea value={description} onChange={(e) => { setDescription(e.target.value); clearError('description'); }} rows={3} className={inputClass(errors.description) + ' resize-none'} />
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 cursor-pointer transition-colors">Cancelar</button>
